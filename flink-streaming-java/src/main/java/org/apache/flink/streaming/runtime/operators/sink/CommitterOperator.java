...
import org.apache.flink.streaming.runtime.operators.sink.committables.CheckpointCommittables;
...
import org.apache.flink.streaming.runtime.operators.sink.committables.Committables;
 ...
 * {@link WriterOperator}, which initially outputs the committables.
 *
...
class CommitterOperator<CommT> extends AbstractStreamOperator<CommittableMessage<CommT>>
    ...
    public void snapshotState(StateSnapshotContext context) throws Exception {
        ...
        // TODO: create copy?, rocksdb should serialize immediately, but for memory state backend,
        // we might need to do a deep clone
        committableCollectorState.update(Collections.singletonList(committableCollector));
    }

    @Override
    public void endInput() throws Exception {
        Committables<CommT> endOfInputCommittables =
                committableCollector.getEndOfInputCommittables();
        // indicates batch
        if (endOfInputCommittables != null) {
            do {
                commitAndEmit(endOfInputCommittables, false);
    ...
    private void commitAndEmitCheckpoints() throws IOException, InterruptedException {
        for (CheckpointCommittables<CommT> checkpoint :
            ...
            // wait for all committables of the current checkpoint before submission
            boolean fullyReceived = checkpoint.getCheckpointId() == lastCompletedCheckpointId;
            commitAndEmit(checkpoint, fullyReceived);
        }
    ...
    private void commitAndEmit(Committables<CommT> committables, boolean fullyReceived)
            throws IOException, InterruptedException {
        Collection<CommittableWithLineage<CommT>> committed =
                committables.commit(fullyReceived, committer);
        if (emitDownstream && !committed.isEmpty()) {
            output.collect(new StreamRecord<>(committables.getSummary()));
