...
public class DynamicFilteringDataCollectorOperatorCoordinator
    ...
    public DynamicFilteringDataCollectorOperatorCoordinator(Context context) {
    ...
    public void handleEventFromOperator(int subtask, int attemptNumber, OperatorEvent event)
            throws Exception {
        //        if (((SourceEventWrapper) event).getSourceEvent() instanceof RuntimeFilterEvent) {
        RuntimeFilterEvent<RowData> filterEvent =
                (RuntimeFilterEvent<RowData>) ((SourceEventWrapper) event).getSourceEvent();
        if (filterEvent.getReceiverType() == ReceiverType.OPERATOR) {
            // push event
            OperatorCoordinator listener =
                    (OperatorCoordinator) coordinatorStore.get(filterEvent.getReceiverUUID());
            if (listener == null) {
            ...
            // TODO to remove, mock as dynamic filter for now
            DynamicFilteringEvent e =
                    new DynamicFilteringEvent(
                            ((DynamicFilteringDataRuntimeFilter) filterEvent.getFilter())
            ...
            listener.handleEventFromOperator(0, 0, new SourceEventWrapper(e));
        }
        //        } else {
        //            DynamicFilteringData currentData =
        //                    ((DynamicFilteringEvent) ((SourceEventWrapper)
        // event).getSourceEvent())
        //                            .getData();
        //                if (receivedFilteringData == null) {
        //                    receivedFilteringData = currentData;
        //                } else {
        //                    // Since there might be speculative execution or failover, we may
        // receive
        //         multiple
        //                    // notifications, and we can't tell for sure which one is valid
        // for further
        //         processing.
        //                    if (DynamicFilteringData.isEqual(receivedFilteringData,
        // currentData)) {
        //                        // If the notifications contain exactly the same data,
        // everything is
        //         alright, and
        //                        // we don't need to send the event again.
        //                        return;
        //                    } else {
        //                        // In case the mismatching of the source filtering result and
        // the dim
        //         data, which
        //                        // may leads to incorrect result, trigger global failover for
        // fully
        //         recomputing.
        //                        throw new IllegalStateException(
        //                                "DynamicFilteringData is recomputed but not equal. "
        //                                        + "Triggering global failover in case the
        // result is
        //         incorrect. "
        //                                        + " It's recommended to re-run the job with
        // dynamic
        //         filtering disabled.");
        //                    }
        //                }
        ...
        //            DynamicFilteringEvent filterEvent =
        //                    (DynamicFilteringEvent) ((SourceEventWrapper)
        // event).getSourceEvent();
    ...
    public static class Provider extends RecreateOnResetOperatorCoordinator.Provider {
        ...
        protected OperatorCoordinator getCoordinator(Context context) throws Exception {
            return new DynamicFilteringDataCollectorOperatorCoordinator(context);
        }
    }
}
